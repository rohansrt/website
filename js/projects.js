(function () {
  ...
  function renderSelfProjectCard(data) {
  ...
  function renderSelfProjectModal(data) {
    var d = data.detail;
    var taglineHTML = esc(data.tagline).replace(/\n/g, '<br>');
  ...
  function renderProfessionalProjectCard(data) {
    var stripsHTML = data.card.strips.map(function (strip) {
      return '<div class="eg-strip-item" title="' + esc(strip.label) + '">' +
        '<img src="' + esc(strip.icon) + '" alt="' + esc(strip.label) + '" class="eg-strip-icon"/>' +
        '<span class="eg-strip-label">' + esc(strip.label) + '</span>' +
  ...
  function renderProfessionalProjectModal(data) {
    var d = data.detail;
    var taglineHTML = esc(data.tagline).replace(/\n/g, '<br>');
  ...
  function loadFullProject(slot, id) {
    fetch('assets/projects/' + id + '.json')
      ...
      .then(function (data) {
        var isPro = data.projectType === 'professional';
        var cardEl = isPro ? renderProfessionalProjectCard(data) : renderSelfProjectCard(data);
        var modalEl = isPro ? renderProfessionalProjectModal(data) : renderSelfProjectModal(data);
